...
def transformer_block(x, num_heads=4, key_dim=32, ff_dim=128):
    ...
    attn_output = Add()([attn_output, x])  # Skip connection
    # Feed-forward network
    ff_output = Dense(ff_dim, activation='relu')(attn_output)
    ff_output = Dense(x.shape[-1])(ff_output)
    return Add()([ff_output, attn_output])  # Skip connection

def segnet_decoder(f, n_classes, n_up=3,btrans= True):
  
    assert n_up >= 2

    o = f
    if btrans== True:
       o = transformer_block(o)
...
def _segnet(n_classes, encoder,  input_height=416, input_width=608,
            encoder_level=3, channels=3,btrans= True):

    img_input, levels = encoder(
        input_height=input_height,  input_width=input_width, channels=channels,btrans= True)

    feat = levels[encoder_level]
    o = segnet_decoder(feat, n_classes, n_up=3,btrans= True)
...
def segnet(n_classes, input_height=416, input_width=608, encoder_level=3, channels=3):
    ...
    model = _segnet(n_classes, vanilla_encoder,  input_height=input_height,
                    input_width=input_width, encoder_level=encoder_level, channels=channels,btrans= True)
